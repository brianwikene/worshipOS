...
<script lang="ts">
	...
	const authItem = $derived({ href: loginHref, label: 'Log in' });
	...
	const activeTenantName = $derived(
		tenants.find((t: Tenant) => t.id === activeChurchId)?.name ??
			(activeChurchId ? 'Selected' : 'None')
	);
	...
	const activeLink = 'text-white border-white';
	const idleLink = 'text-white/70 hover:text-white hover:border-white/40';

	const dropdownPanel =
		'absolute left-0 mt-2 w-56 bg-[#111317] border border-white/10 rounded-md shadow-xl overflow-hidden z-50';
	const dropdownItem = 'block px-3 py-2 text-sm text-white/80 hover:bg-white/5 hover:text-white';
</script>
...
<header class="w-full sticky top-0 z-50 bg-[#1f2428] border-b border-white/10">
				...
				<nav class="hidden md:block min-w-0" aria-label="Primary">
					<ul class="flex flex-wrap items-center gap-5">
						...
						<li class="relative group">
							...
							<div
								class={`hidden group-hover:block ${dropdownPanel}`}
								role="menu"
	...
	{#if mobileOpen}
		<div
			id="mobile-menu"
			bind:this={menuEl}
			class="md:hidden border-t border-white/10 bg-[#1f2428]"
		>
			<nav aria-label="Mobile Primary" class="sys-shell sys-shell--mobile">
				<ul class="flex flex-col gap-1">
					<li>
						<a
							href="/start"
							class="block px-3 py-2 rounded-md text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white"
							onclick={closeMobileMenu}>Start</a
						>
					</li>
					<li>
						<a
							href="/gatherings"
							class="block px-3 py-2 rounded-md text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white"
							onclick={closeMobileMenu}>Gatherings</a
						>
					</li>
					...
					<li class="mt-1">
						...
						{#each connections.items as item}
							<a
								href={item.href}
								class="block px-6 py-2 rounded-md text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white"
					...
					<li>
						<a
							href="/songs"
							class="block px-3 py-2 rounded-md text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white"
							onclick={closeMobileMenu}>Songs</a
						>
					</li>
					...
					<li>
						<a
							href={authItem.href}
							class="block px-3 py-2 rounded-md text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white"
							onclick={closeMobileMenu}>{authItem.label}</a
						>
					</li>
					...
					<li>
						<div class="px-3 py-2">
							<label
								class="block text-xs font-semibold text-white/60 mb-1"
								for="tenant-select-mobile"
							>
								Active church
							</label>
