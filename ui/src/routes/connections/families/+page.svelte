...
<div class="sys-page">
	<div class="sys-page-header">
		...
		<button class="sys-btn sys-btn--primary" onclick={openAddModal}> + Add Family </button>
	</div>
	...
	{:else if errorMsg}
		<div class="sys-state sys-state--error">
			<p>Error: {errorMsg}</p>
			<button class="sys-btn sys-btn--danger" onclick={loadFamilies}>Retry</button>
		</div>
		...
		<div class="sys-state sys-state--empty">
			...
			{:else}
				...
				<button class="sys-btn sys-btn--primary" onclick={openAddModal}
					>Create your first family</button
				>
			{/if}
		</div>
			...
			{#each families as family}
				<div class="sys-card family-card">
					...
					<div class="card-actions">
						<button class="sys-icon-btn" onclick={() => openEditModal(family)} title="Edit">
							✏️
						</button>
						<button
							class="sys-icon-btn sys-icon-btn--danger"
							onclick={() => handleArchive(family)}
							title="Archive"
						>
							🗑️
						</button>
							...
							<tr>
								<td>
									<a href={`/connections/families/${family.id}`} class="row-link">
										<ObjectMark variant="families" size="sm" label={getInitials(family.name)} />
									</a>
								</td>
