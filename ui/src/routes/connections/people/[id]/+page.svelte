<!-- /ui/src/routes/connections/people/[id]/+page.svelte -->
...
<script lang="ts">
	...
	$: rolesByMinistry = unassignedRoles.reduce(
		(acc, role) => {
	...
	$: capabilitiesByMinistry = capabilities.reduce(
		(acc, cap) => {
...
<div class="sys-page sys-page--narrow">
	...
	{:else if error}
		<div class="sys-state sys-state--error">
			<p>Error: {error}</p>
			<button class="sys-btn sys-btn--danger" onclick={loadPerson}>Retry</button>
		</div>
	{:else if person}
		<div class="profile-card">
			<div class="profile-header">
				...
				<div class="header-actions">
					<button class="sys-btn sys-btn--primary" onclick={openEditModal}> ✏️ Edit </button>
					<button class="sys-btn sys-btn--secondary btn-archive" onclick={handleArchive}>
						🗑️ Archive
					</button>
				</div>
			</div>
			...
			<div class="info-section">
				...
				<div class="section-header-row">
					<h2>Addresses</h2>
					<button
						class="sys-btn sys-btn--primary btn-small"
						onclick={openAddAddressModal}
						title="Add a personal address (e.g., college, work)"
					>
				...
				{#if person.family_addresses.length === 0 && person.addresses.length === 0}
					<div class="empty-row">
						...
						<button class="btn-link" onclick={openAddAddressModal}>Add a personal address</button>
					</div>
				{:else}
					...
					{#each person.family_addresses as addr}
						<div class="info-row address-row family-address">
							...
							<div class="row-actions">
								<a
									href="/connections/families/{addr.family_id}"
									class="btn-icon"
									title="Edit on family page">↗️</a
								>
							</div>
						</div>
					{/each}
					...
					{#each person.addresses as addr}
						<div class="info-row address-row personal-address">
							...
							<div class="row-actions">
								<button class="btn-icon" onclick={() => openEditAddressModal(addr)} title="Edit"
									>✏️</button
								>
								<button class="btn-icon delete" onclick={() => deleteAddress(addr)} title="Delete"
									>🗑️</button
								>
							</div>
						</div>
				...
				{#if capabilities.length === 0}
					<div class="empty-roles-card">
						...
						<button class="sys-btn sys-btn--primary" onclick={() => (showAddRoleModal = true)}>
									...
									<div class="role-card" class:primary={cap.is_primary}>
										...
										<div class="role-actions">
											<button
												...
												onclick={() => togglePrimary(cap.role_id, cap.is_primary)}
											...
											<button
												class="btn-remove"
												onclick={() => removeCapability(cap.role_id, cap.role_name)}
												title="Remove role"
											>
												×
											</button>
...
{#if showAddRoleModal}
	<div
		...
		onclick={() => (showAddRoleModal = false)}
		onkeydown={(e) => e.key === 'Escape' && (showAddRoleModal = false)}
	>
		<div
			class="sys-modal"
		...
		>
			<div class="sys-modal-header">
				...
				<button class="sys-modal-close" onclick={() => (showAddRoleModal = false)}>×</button>
			</div>
			...
			<div class="sys-modal-body">
				{#if unassignedRoles.length === 0}
					<div class="no-roles-available">
						<p>
							This person has been assigned all available roles, or no roles have been defined yet.
						</p>
					</div>
				{:else}
			...
			<div class="sys-modal-actions">
				<button class="sys-btn sys-btn--secondary" onclick={() => (showAddRoleModal = false)}
					>Cancel</button
				>
				<button
	...
	>
		...
		>
			<div class="sys-modal-header">
				...
				<button class="sys-modal-close" onclick={closeAddressModal}>×</button>
			</div>
			...
			<div class="sys-modal-actions">
				<button class="sys-btn sys-btn--secondary" onclick={closeAddressModal}>Cancel</button>
				<button class="sys-btn sys-btn--primary" onclick={saveAddress} disabled={savingAddress}>
...
<style>
	...
	.avatar-large {
		...
		background: linear-gradient(
			135deg,
			var(--gatherings-accent-start),
			var(--gatherings-accent-end)
		);
		color: white;
		border-radius: 50%;
		display: flex;
