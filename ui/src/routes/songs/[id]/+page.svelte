<script lang="ts">
	...
	function parseChordPro(text: string | null): { lines: Line[]; meta: ExtractedMeta } {
		...
		for (let line of rawLines) {
			...
			if (trimmed.startsWith('{title:') || trimmed.startsWith('{t:')) {
				meta.title = extractTag(trimmed);
				continue;
			}
			if (
				trimmed.startsWith('{subtitle:') ||
				trimmed.startsWith('{st:') ||
				trimmed.startsWith('{su:')
			) {
				meta.subtitle = extractTag(trimmed);
				continue;
			}
			if (trimmed.startsWith('{artist:') || trimmed.startsWith('{a:')) {
				meta.artist = extractTag(trimmed);
				continue;
			}
			if (trimmed.startsWith('{key:') || trimmed.startsWith('{k:')) {
				meta.key = extractTag(trimmed);
				continue;
			}
			if (trimmed.startsWith('{time:')) {
				meta.time = extractTag(trimmed);
				continue;
			}
			if (trimmed.startsWith('{tempo:') || trimmed.startsWith('{bpm:')) {
				meta.tempo = extractTag(trimmed);
				continue;
			}
			if (
				trimmed.startsWith('{copyright:') ||
				(trimmed.startsWith('{c:') && trimmed.toLowerCase().includes('copyright'))
			) {
				meta.copyright = extractTag(trimmed);
				continue;
			}
			if (trimmed.startsWith('{ccli:')) {
				meta.ccli = extractTag(trimmed);
				continue;
			}
			...
			if (trimmed.match(/^\{/)) {
				if (
					trimmed.match(/^\{(c:|comment:|soc|start_of_chorus|verse|chorus|bridge|intro|outro)/i)
				) {
					let label = trimmed
						.replace(/^\{(c:|comment:|soc|start_of_chorus|eoc|end_of_chorus)/i, '')
						.replace(/[{}\[\]]/g, '')
						.trim();
	...
	async function saveLyrics() {
		...
		try {
			...
			if (meta.tempo) {
				const nums = meta.tempo.match(/\d+/);
				if (nums) payload.bpm = parseInt(nums[0]);
			}
	...
	function handleSchedule() {
		alert('Added to Gathering! (Simulation)');
	}
	function startEditing() {
		if (song) {
			editBuffer = song.raw_text || '';
			isEditing = true;
		}
	}
	...
	{:else if song}
		<div class="sys-page-header no-print">
			<div style="display: flex; gap: 16px; align-items: center;">
				...
				<div>
					...
					<p class="sys-subtitle">
						{computedMeta.artist || song.artist || 'Unknown'} • {computedMeta.key ||
							song.key ||
							'No Key'}
					</p>
				</div>
			...
			<div class="header-actions">
				<button class="sys-btn sys-btn--primary" onclick={handleSchedule}>+ Schedule</button>
				<button class="sys-btn sys-btn--secondary" onclick={openEditModal}>Edit Details</button>
			</div>
		</div>
		...
		<div class="sys-tabs no-print">
			<button
				...
				onclick={() => (activeTab = 'chart')}>Chart</button
			>
			<button
				class="sys-tab"
				class:active={activeTab === 'lyrics'}
				onclick={() => (activeTab = 'lyrics')}>Lyrics</button
			>
			<button
				class="sys-tab"
				class:active={activeTab === 'resources'}
				onclick={() => (activeTab = 'resources')}>Files ({song.attachments?.length || 0})</button
			>
			<button
				class="sys-tab"
		...
		<div class="content-grid">
						...
						<div class="editor-grid no-print">
							<div class="sys-card editor-card">
								...
								<div class="editor-actions">
									<button class="sys-btn sys-btn--secondary" onclick={() => (isEditing = false)}
										>Cancel</button
									>
									<button class="sys-btn sys-btn--primary" onclick={saveLyrics}>Save Changes</button
									>
								</div>
							</div>
							...
							<div class="tag-helper">
								...
								<div class="tag-group">
									<label>Headers</label>
									<button class="tag-btn" onclick={() => insertTag('{c: Verse 1}\n')}
										>Verse 1</button
									>
									<button class="tag-btn" onclick={() => insertTag('{c: Chorus}\n')}>Chorus</button>
									<button class="tag-btn" onclick={() => insertTag('{c: Bridge}\n')}>Bridge</button>
								</div>
								<div class="tag-group">
									<label>Metadata</label>
									<button class="tag-btn" onclick={() => insertTag('{key: G}\n')}>Key: G</button>
									<button class="tag-btn" onclick={() => insertTag('{time: 4/4}\n')}
										>Time: 4/4</button
									>
									<button class="tag-btn" onclick={() => insertTag('{tempo: 72}\n')}>BPM: 72</button
									>
								</div>
								<div class="tag-group">
									<label>Chords</label>
									<button class="tag-btn" onclick={() => insertTag('[G]')}>[G]</button>
									<button class="tag-btn" onclick={() => insertTag('[C]')}>[C]</button>
									<button class="tag-btn" onclick={() => insertTag('[D]')}>[D]</button>
								</div>
							</div>
						</div>
					{:else}
						<div class="chart-controls no-print">
							...
							<div class="chart-actions">
								<button class="sys-btn sys-btn--sm sys-btn--secondary" onclick={handlePrint}
									>🖨️ Print</button
								>
								<button class="sys-btn sys-btn--sm sys-btn--primary" onclick={startEditing}
									>✏️ Edit</button
								>
							</div>
						</div>
								...
								{#each pages as pageLines, i}
									<div class={`paper-sheet size-${fontSize}`}>
										<div class="sheet-header">
											{#if i === 0}
												<div class="sheet-main-info">
													...
													{#if computedMeta.subtitle}<p class="sheet-subtitle">
															{computedMeta.subtitle}
														</p>{/if}
										...
										<div class="sheet-footer">
											{#if computedMeta.ccli || song.ccli_number}CCLI #{computedMeta.ccli ||
													song.ccli_number} •
											{/if}
											{computedMeta.copyright ||
												`© ${new Date().getFullYear()} ${song.artist || 'Copyright'}`}
											<span class="footer-page"> • Page {i + 1}/{pages.length}</span>
										</div>
									</div>
								{/each}
							{:else}
								<div class="paper-sheet size-md empty-chart" onclick={startEditing}>
				...
				{:else if activeTab === 'lyrics'}
					<div class="sys-card">
						...
						<p class="text-sm text-muted mb-4">
							Auto-generated from your chord chart for vocalists.
						</p>
							...
							{#each song.attachments || [] as file}
								<div class="file-row">
									...
									<button
										...
										onclick={() => deleteAttachment(file.id)}>🗑️</button
									>
								</div>
							{/each}
						</div>
						<button
							class="sys-btn sys-btn--secondary mt-4"
			...
			<div class="sidebar no-print">
				<div class="sys-card meta-card">
					<h3>Quick Look</h3>
					<div class="meta-row">
						<label>BPM</label> <span>{computedMeta.tempo || song.bpm || '—'}</span>
					</div>
					<div class="meta-row">
						<label>Key</label> <span>{computedMeta.key || song.key || '—'}</span>
					</div>
					<div class="meta-row">
						<label>CCLI</label> <span>{computedMeta.ccli || song.ccli_number || '—'}</span>
					</div>
				</div>
				<div class="danger-zone">
					<button class="sys-btn-link text-red" onclick={handleIntegrityAction}>
