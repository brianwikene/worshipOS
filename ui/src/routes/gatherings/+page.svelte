...
<script lang="ts">
	...
	// --- State ---
	let view = $state<ViewMode>('upcoming');
	...
	let contexts = $state<Array<{ id: string; name: string }>>([]);
	let campuses = $state<Array<{ id: string; name: string }>>([]);

	// Form state
	let newServiceName = $state('');
	let newServiceContextId = $state('');
	let newServiceDate = $state('');
	let serviceInstances = $state<Array<{ time: string; campus_id: string }>>([
		{ time: '09:00', campus_id: '' }
	]);
	...
	// Keep view in sync with URL (?view=past)
	$effect(() => {
	...
	async function loadFormData() {
		try {
			...
			const [ctxData, campusData] = await Promise.all([
				apiJson<Array<{ id: string; name: string }>>('/api/contexts'),
				apiJson<Array<{ id: string; name: string }>>('/api/campuses')
			]);
			contexts = ctxData;
			campuses = campusData;
		} catch (e) {
	...
	const visibleServices = $derived.by(() => {
		const list = (services as ServiceGroup[])
			.filter((service) =>
				view === 'past' ? isPastGroupDate(service.group_date) : !isPastGroupDate(service.group_date)
			)
			.sort((a, b) =>
				view === 'past'
	...
	<header>
		<div class="header-content">
			<div class="title-section">
				<h1>Gatherings</h1>
				<p>Build the plan. Share it with the team</p>
			</div>
			...
			<div class="header-actions">
				<button class="sys-btn sys-btn--primary" type="button" onclick={openAddModal}>
					+ Add Gathering
				</button>
				<button
										...
										<div class="assignments-container">
											...
											<div class="total-progress">
												<div class="progress-text">
													{instance.assignments.filled_positions} of {instance.assignments
														.total_positions} positions filled
												</div>
												<div class="progress-track">
													<div
														class="progress-fill"
														style="width: {(instance.assignments.filled_positions /
															instance.assignments.total_positions) *
															100}%"
													></div>
												</div>
											</div>
														...
														<div class="ministry-row">
															<div class="ministry-info">
																...
																<div class="ministry-stats">
																	...
																	{/if}
																	<span class="ministry-count"
																		>{ministry.confirmed}/{ministry.total} confirmed</span
																	>
																</div>
															</div>
															<div class="ministry-track">
																<div
																	class="ministry-fill green"
																	style="width: {(ministry.confirmed / ministry.total) * 100}%"
																></div>
																<div
																	class="ministry-fill yellow"
																	style="width: {(ministry.pending / ministry.total) * 100}%"
																></div>
															</div>
														</div>
													{/each}
	...
	>
		...
		>
			<div class="modal-header">
				...
				<button class="close-btn" type="button" aria-label="Close modal" onclick={closeAddModal}
					>×</button
				>
			</div>
				...
				<div class="form-section">
					...
					<div class="form-group">
						...
						<small class="help-text"
							>Gatherings can only be scheduled for today or future dates.</small
						>
					</div>
				</div>
				...
				<div class="form-section">
					...
					{#each serviceInstances as instance, idx}
						...
						<div class="instance-row">
							...
							{#if serviceInstances.length > 1}
								<button type="button" class="remove-btn" onclick={() => removeServiceInstance(idx)}
									>×</button
								>
							{/if}
						</div>
			...
			<div class="modal-actions">
				<button class="secondary-btn" type="button" onclick={closeAddModal}>Cancel</button>
				<button
...
<style>
	...
	.instance-row .form-group {
		margin-bottom: 0;
		flex: 1;
	}
	.instance-row .flex-grow {
		flex: 2;
	}
