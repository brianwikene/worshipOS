...
<main style="padding: 1rem; max-width: 900px; margin: 0 auto;">
	...
	>
		...
		<div style="display:flex; gap:.75rem; align-items:center; flex-wrap:wrap;">
			<label style="display:flex; gap:.5rem; align-items:center;">
				...
				<select bind:value={activeChurchId} onchange={onTenantChange}>
					{#each tenants as t}
			...
			<span
				style="font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:.85rem; opacity:.75;"
			>
				{activeChurchId}
			</span>
		</div>
	...
	<section style="margin-top: 1.25rem;">
		...
		{:else if error}
			...
			<p style="opacity:.75;">
				Sanity check: your browser should be sending <code>X-Church-Id</code> on every request, and the
				API should reject requests missing it.
			</p>
