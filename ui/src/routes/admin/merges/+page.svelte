...
<script lang="ts">
	...
	function formatTransferred(transferred: Record<string, number>): string {
		const parts = [];
		if (transferred.service_assignments)
			parts.push(`${transferred.service_assignments} assignments`);
		if (transferred.role_capabilities) parts.push(`${transferred.role_capabilities} capabilities`);
		if (transferred.family_memberships)
			parts.push(`${transferred.family_memberships} family memberships`);
...
<div class="container">
	...
	{:else if error}
		<div class="error">
			<p>{error}</p>
			<button onclick={loadMerges}>Retry</button>
		</div>
			...
			{#each merges as merge}
				<div class="merge-card" class:undone={merge.undone_at}>
					...
					<div class="merge-actions">
						{#if merge.can_undo}
							<button class="btn-undo" onclick={() => openUndo(merge)}> Undo Merge </button>
	...
	>
		<div class="modal" onclick={(e) => e.stopPropagation()} onkeydown={() => {}}>
			<div class="modal-header">
				<h2>Undo Merge</h2>
				<button class="close-btn" onclick={closeUndo}>×</button>
			</div>
			...
			<div class="modal-body">
				...
				<div class="undo-warning">
					<strong>Note:</strong> Records that were transferred (service assignments, capabilities,
					etc.) will remain with the survivor ({selectedMerge.survivor_name}). You may need to
					manually reassign them if needed.
				</div>
			...
			<div class="modal-actions">
				<button class="btn-cancel" onclick={closeUndo}>Cancel</button>
				<button class="btn-undo-confirm" onclick={performUndo} disabled={undoing}>
