...
<div class="container">
  <div class="header">
    ...
    <div class="header-actions">
      ...
      <button class="btn-primary" on:click={runScan} disabled={scanning}>
  ...
  {#if scanResult}
    <div class="scan-result">
      ...
      <button class="dismiss" on:click={() => scanResult = null}>×</button>
    </div>
  {/if}
  ...
  {:else if error}
    <div class="error">
      <p>{error}</p>
      <button on:click={loadDuplicates}>Retry</button>
    </div>
  {:else if duplicates.length === 0}
    <div class="empty">
      <p>No duplicate candidates found.</p>
      <button class="btn-primary" on:click={runScan}>Run a Scan</button>
    </div>
  {:else}
        ...
        <div class="duplicate-card">
          ...
          <div class="card-actions">
            {#if link.status === 'suggested'}
              <button class="btn-review" on:click={() => openReview(link)}>Review</button>
            {:else}
...
<div class="modal-overlay" on:click={closeReview} on:keydown={(e) => e.key === 'Escape' && closeReview()}>
  <div class="modal review-modal" on:click|stopPropagation on:keydown={() => {}}>
    <div class="modal-header">
      <h2>Review Potential Duplicate</h2>
      <button class="close-btn" on:click={closeReview}>×</button>
    </div>
    ...
    <div class="modal-body">
      {#if !showMergeModal}
        ...
        <div class="notes-section">
          ...
          <textarea id="review-notes" bind:value={reviewNotes} placeholder="Add any notes about this decision..."></textarea>
        </div>
      {:else}
        <!-- Merge View -->
        <div class="merge-wizard">
          <h3>Choose Survivor Record</h3>
          <p class="merge-info">The survivor record will remain active. The other record will be archived but can be restored.</p>

          <div class="survivor-options">
            <label class="survivor-option" class:selected={survivorId === selectedLink.person_a.id}>
              ...
              <div class="option-content">
                ...
                <span class="option-meta">Created {formatDate(selectedLink.person_a.created_at)}</span>
              </div>
            </label>

            <label class="survivor-option" class:selected={survivorId === selectedLink.person_b.id}>
              <input type="radio" bind:group={survivorId} value={selectedLink.person_b.id} />
              <div class="option-content">
                <strong>{selectedLink.person_b.display_name}</strong>
                <span class="option-meta">Created {formatDate(selectedLink.person_b.created_at)}</span>
              </div>
            </label>
          </div>
          ...
          <div class="merge-notes">
            ...
            <textarea id="merge-reason" bind:value={mergeReason} placeholder="e.g., Same person - AJ is Armani's nickname"></textarea>
          </div>
    ...
    <div class="modal-actions">
      {#if !showMergeModal}
        <button class="btn-not-match" on:click={markNotMatch} disabled={saving}>
          Not a Match
        </button>
        <button class="btn-confirm" on:click={confirmDuplicate} disabled={saving}>
          Confirm Duplicate
        </button>
        <button class="btn-merge" on:click={startMerge} disabled={saving}>
          Merge Records →
        </button>
      {:else}
        <button class="btn-secondary" on:click={() => showMergeModal = false}>
          ← Back
        </button>
        <button class="btn-merge" on:click={performMerge} disabled={merging || !survivorId}>
...
<style>
  ...
  .notes-section textarea, .merge-notes textarea {
    width: 100%;
    min-height: 80px;
    padding: 0.75rem;
    border: 1px solid #ddd;
  ...
  .btn-merge:disabled, .btn-confirm:disabled, .btn-not-match:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }
