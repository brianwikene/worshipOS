...
<script lang="ts">
	...
	$: filteredRoles = filterTeamId ? roles.filter((r) => r.team_id === filterTeamId) : roles;

	$: rolesByTeam = filteredRoles.reduce(
		(acc, role) => {
...
<div class="sys-page">
	<div class="sys-page-header">
		...
		<button class="sys-btn sys-btn--primary" onclick={openAddModal}> + Add Role </button>
	</div>
	...
	{:else if error}
		<div class="sys-state sys-state--error">
			<p>{error}</p>
			<button class="sys-btn sys-btn--danger" onclick={loadRoles}>Retry</button>
		</div>
						...
						{#each teamGroup.roles as role}
							<div class="role-card" class:inactive={!role.is_active}>
								...
								<div class="role-actions">
									<button class="btn-edit" onclick={() => openEditModal(role)}> Edit </button>
									<button
										class="btn-toggle"
			...
			<div class="sys-modal-body">
				...
				<div class="sys-form-group">
					...
					<p class="sys-help-text">
						Select which body parts this role uses. This helps prevent scheduling conflicts (e.g.,
						can't play drums and guitar at the same time).
					</p>
				...
				<div class="sys-form-group">
					...
					<div class="load-weight-input">
						<input id="load-weight" type="range" min="1" max="20" bind:value={formLoadWeight} />
						<span class="load-value">{formLoadWeight}</span>
					</div>
					<small class="sys-help-text"
						>Higher weight = more demanding role. Used for balancing schedules.</small
					>
				</div>
			</div>
