...
<script lang="ts">
	...
	async function toggleActive(team: Team) {
		...
		const roleWarning =
			team.roles.length > 0 ? ` This will unassign ${team.roles.length} role(s).` : '';
...
<div class="sys-page">
	<div class="sys-page-header">
		<div>
			...
			<p class="sys-subtitle">
				Organize roles into teams like Worship, Hospitality, Kids, Tech, etc.
			</p>
		</div>
		<button class="sys-btn sys-btn--primary" onclick={openAddModal}> + Add Team </button>
	</div>
	...
	{:else if error}
		<div class="sys-state sys-state--error">
			<p>{error}</p>
			<button class="sys-btn sys-btn--danger" onclick={loadTeams}>Retry</button>
		</div>
				...
				<div class="team-card">
					<div class="team-card-header">
						<div
							...
							style="color: {team.color}; background-color: {team.color}15;"
						>
							{team.icon}
						</div>
						<h2>{team.name}</h2>
					...
					<div class="team-card-footer">
						<button class="btn btn-sm" onclick={() => openEditModal(team)}> Edit </button>

						<a href="/admin/roles?team={team.id}" class="btn btn-sm"> Manage Roles </a>

						<button class="btn btn-sm hover:text-red-600" onclick={() => toggleActive(team)}>
							Deactivate
						</button>
					</div>
				</div>
				...
				<div class="inactive-list">
					{#each inactiveTeams as team}
						<div class="inactive-team">
							...
							<button class="btn-reactivate" onclick={() => toggleActive(team)}>
								Reactivate
							</button>
						</div>
					{/each}
		...
		>
			...
			<div class="sys-modal-body p-6">
				...
				<div class="sys-form-group mb-6">
					<label for="team-desc" class="block text-sm font-medium text-gray-700 mb-1"
					...
					<div class="sys-form-group">
						...
						<div class="picker-grid grid grid-cols-5 gap-2">
							{#each availableIcons as icon}
								<button
									type="button"
									class="w-10 h-10 flex items-center justify-center rounded-lg border transition-all {formIcon ===
									icon
					...
					<div class="sys-form-group">
						...
						<div class="picker-grid grid grid-cols-5 gap-2">
							{#each availableColors as color}
								<button
									type="button"
									class="w-10 h-10 rounded-full flex items-center justify-center transition-transform {formColor ===
									color
			...
			<div
				class="sys-modal-actions border-t border-gray-100 p-4 bg-gray-50 rounded-b-xl flex justify-end gap-3"
			>
				<button
