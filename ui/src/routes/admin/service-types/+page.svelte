<!-- /ui/src/routes/admin/service-types/+page.svelte -->
<script lang="ts">
	...
	async function loadServiceTypes() {
		...
		error = '';
		try {
			serviceTypes = await apiJson<ServiceType[]>(
				'/api/contexts?include_inactive=true&include_requirements=true'
			);
		} catch (e: any) {
...
<div class="sys-page">
	<div class="sys-page-header">
		...
		<button class="sys-btn sys-btn--primary" onclick={openAddModal}> + Add Service Type </button>
	</div>
	...
	{:else if error}
		<div class="sys-state sys-state--error">
			<p>{error}</p>
			<button class="sys-btn sys-btn--danger" onclick={loadServiceTypes}>Retry</button>
		</div>
			...
			{#each serviceTypes as type}
				<div class="type-card" class:inactive={!type.is_active}>
								...
								{#each type.role_requirements as req}
									<div class="requirement-item">
										...
										<span class="req-count">
											{req.min_needed}{req.max_needed && req.max_needed !== req.min_needed
												? `-${req.max_needed}`
												: ''}
										</span>
									</div>
					...
					<div class="type-actions">
						<button class="btn-edit" onclick={() => openEditModal(type)}> Edit </button>
						<button
							class="btn-toggle"
	...
	<div class="sys-modal-overlay">
		...
		>
				...
				<div class="form-section">
					<div class="section-header">
						...
						<span class="position-count"
							>{formRequirements.reduce((s, r) => s + r.min_needed, 0)} total</span
						>
					</div>
					<p class="sys-help-text">
						Define which roles are needed for this type of service and how many of each.
					</p>
					...
					{:else}
						<div class="no-roles-warning">
							<p>
								No roles defined yet. <a href="/admin/roles">Create roles first</a> to add position requirements.
							</p>
						</div>
					{/if}
				</div>
			...
			<div class="sys-modal-actions">
				<button class="sys-btn sys-btn--secondary" onclick={closeModal}>Cancel</button>
				<button
