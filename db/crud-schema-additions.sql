...
SELECT 
  ...
  srr.display_order,
  CASE 
    WHEN srr.is_required THEN '✓ Required'
    ELSE 'Optional'
...
SELECT 
  ...
  CASE 
    WHEN srr.is_required AND COUNT(sa.id) FILTER (WHERE sa.person_id IS NOT NULL) < srr.min_needed 
      THEN '🔴 UNFILLED'
...
GROUP BY 
  ...
  srr.min_needed, srr.max_needed, srr.is_required
ORDER BY 
...
-- Test copying Dec 21 to Jan 4
/*
SELECT copy_service_to_date(
  (SELECT si.id FROM service_instances si
   JOIN service_groups sg ON sg.id = si.service_group_id
   WHERE sg.group_date = '2025-12-21' AND si.service_time = '09:00:00'
   LIMIT 1),
